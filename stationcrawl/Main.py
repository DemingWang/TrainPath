...
def start(arg):
    ...
    """先抓取火车车次信息json"""
    # train_no_url = TRAIN_NO_BASE_URL_NEW + 'linktypeid=dc' + '&fs=' + str(arg[1]) + '&ts=' + str(arg[2]) + '&data=' + '2020-02-16' + '&flag=N,N,Y'
    ...
    # https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=2019-08-16&leftTicketDTO.from_station=FZS&leftTicketDTO.to_station=CQW&purpose_codes=ADULT
    # https://kyfw.12306.cn/otn/leftTicket/query?leftTicketDTO.train_date=2020-02-16&leftTicketDTO.from_station=SHH&leftTicketDTO.to_station=BJP&purpose_codes=ADULT
    # https://kyfw.12306.cn/otn/leftTicket/queryZ?leftTicketDTO.train_date=2020-02-16&leftTicketDTO.from_station=SHH&leftTicketDTO.to_station=BJP&purpose_codes=ADULT
    print("train_no_url:", train_no_url)


    train_no_text = HttpUtils.get_bytes(train_no_url,arg)
    print(train_no_text)
    ...
    for train_no in parse_train_no_json(train_no_text):
        ...
        station_info_text = HttpUtils.get_bytes(station_info_url,arg)
        # 解析沿途停靠站点信息
        ...
        if arg[2] != STATION_DICT[station_info_list[2]]:
            ...
            FileUtils.save_file(path, station_info_text.encode('utf-8'))
